...
const allMockTenders = [
  // Renamed to allMockTenders as we'll filter this
  {
    id: "tender-1",
...
const sriLankanLocations = [
  ...
  },
  {
    ...
    count: 1043,
    districts: [{ name: "Kurunegala" }, { name: "Puttalam" }].sort((a, b) =>
  ...
  },
  {
    ...
    count: 787,
    districts: [{ name: "Anuradhapura" }, { name: "Polonnaruwa" }].sort(
  ...
  },
  {
    ...
    count: 750,
    districts: [{ name: "Kegalle" }, { name: "Ratnapura" }].sort((a, b) =>
      a.name.localeCompare(b.name)
    ),
  },
  {
    ...
    count: 731,
    districts: [{ name: "Badulla" }, { name: "Monaragala" }].sort((a, b) =>
      a.name.localeCompare(b.name)
    ),
  },
];
...
const TenderListingSection: React.FC = () => {
  ...
  const getClosingDateColor = (days: number) => {
    if (days <= 2) {
      // Less than or equal to 2 days remaining (or expired)
      return "text-[var(--color-accent-red)]"; // Use accent red
    } else if (days <= 10) {
      // 3 to 10 days remaining
            ...
            const item = {
              label:
                filterKey === "all"
                  ...
                  ? "Live Tenders"
                  : "Closed Tenders",
              count: allMockTenders.filter((tender) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-[var(--color-dark)] text-white p-6 rounded-lg text-center shadow-md">
              <p className="text-lg font-semibold font-body mb-3">
                To Be Connect with Buyer's & Supplies
              </p>
            ...
            <Button
              label="Registration of Suppliers (2790)"
              className="w-full bg-[var(--color-accent-orange-dark)] hover:bg-[var(--color-accent-orange-dark)]/80 text-white shadow-md py-3 text-lg font-semibold"
            />
                ...
                {tenderCategories.map((category, index) => (
                  <li key={index}>
                    <div className="flex justify-between items-center p-3 hover:bg-gray-50 cursor-pointer transition-colors duration-200">
                      <span className="text-[var(--color-dark)] font-medium font-body">
                  ...
                  <li key={index}>
                    <div
                      ...
                      onClick={() =>
                        ...
                        toggleProvince(location.province)
                      }
                    >
                      <span className="text-[var(--color-dark)] font-medium font-body">
                        {location.province} ({location.count})
                      </span>
                      {location.districts &&
                        location.districts.length > 0 &&
                        (openProvinces[location.province] ? (
                      ...
                      openProvinces[location.province] && (
                        <ul className="bg-gray-50 border-t border-gray-100">
                          {location.districts.map((district, distIndex) => (
                            <li
                              key={distIndex}
                              className="flex justify-between items-center p-2 pl-8 hover:bg-gray-100 cursor-pointer transition-colors duration-200 text-sm font-body"
                            >
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="text-sm text-gray-600 mb-4 font-body">
              Showing 1 - {filteredTenders.length} of {allMockTenders.length}{" "}
              total tenders
            </div>
            ...
            {filteredTenders.map((tender) => (
              ...
              >
                ...
                <div className="flex flex-wrap justify-between items-center mt-4 pt-4 border-t border-gray-100">
                  ...
                  <span
                    className={`text-sm font-semibold font-body inline-block mb-2 sm:mb-0 ${getClosingDateColor(
                      tender.closingInDays
                    )}`}
                  >
                    Tender Closing in: {tender.closingInDays} Day
                    {tender.closingInDays !== 1 ? "s" : ""}
                  </span>
                  <Button
                    label="Click To View"
                    className="bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/80 shadow-md inline-block text-sm py-2 px-4 rounded-md"
                    onClick={() =>
                      console.log(`Navigating to tender ${tender.id}`)
                    }
                  />
                </div>
              </div>
